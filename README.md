# 源亜明朝について
この源亜(げんあ) 明朝は、Amazon Kindle Paperwhite/Oasis上で[Noto Serif CJK JP](https://github.com/notofonts/noto-cjk)を極力適用する為に改良をしたものです。  
...
Noto Serif CJK JPを使用することを推奨します。  
名前の由来は基のフォントの名前「源」と亜馬孫/亜馬生(アマゾン)、亜種の「亜」から取っています。<s>語呂が悪い。</s>  

# フォントの使い方
\\\\Kindle\\fonts フォルダに、2つの GenAMinchoVertical-\*\*.ttf を入れると縦書き用、 GenAUDMinchoHorizontal-\*\*.ttf を入れると横書き用がKindle上で設定できるようになります。  
...
# フォントの特徴  
標準(Regular)については、基となったNoto Serif CJK JPのグリフを一切変えていません。太字・斜体フォントと互換性を持たせるため、フォント名等の設定調整に留めています。  
...
# 謝辞
Noto Sans/Serif CJK JPを配布してくださっている[notofonts](https://github.com/notofonts)様  
